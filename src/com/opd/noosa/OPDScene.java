...
public class OPDScene extends Scene {
	...
	protected void displayVersion(int w, int h) {
		displayVersion(this, w, h, 0, 0);
	}

	public static void displayVersion(Group c, int w,
			int y) {
		displayVersion(c, w, -1, 0, y);
	}

	public static void displayVersion(Group c, int w, int h,
			int x, int y) {
		...
		gameName.x = x + (w > -1 ? w - GAP - gameName.width() : GAP) - 1;
		gameName.y = y + (h > -1 ? h - GAP - gameName.height() - version.height() : GAP);
		c.add( gameName );
		
		version.x = x + (w > -1 ? w - GAP - version.width() : GAP);
		version.y = y + (h > -1 ? h - GAP - version.height() : GAP + gameName.height());
		c.add( version );
	}
}
